...
# McInerny et al. method, see ref(1)
term <- function(tn, n, alpha = 0.05, exp = 3) {
...
# iterative steps are introduced..
# for performance optimization
# the first iteration uses time steps of 10^exp
# the exponent decreases by 1 after each iteration..
# until it reaches 0


# IWM (Inverse Weighted McInerny et al. method), see ref(3)
IWM <- function(sightings, alpha = 0.05, exp = 3) {
  ...
  for(k in 2:n) {
    ti     <- sightings[k]
    tn     <- ti - t1
    thetas <- c(thetas, term(tn, k, alpha, exp))
...
# alpha and exp values are passed on to the term() function
...
GRIWM <- function(sightings, sdevs, iterations = 10000, 
  ...
  for (i in 1:iterations) {
    terms[i] <- IWM(m[i, ], alpha, exp)$term
    if (!silent) {
...
# alpha and exp values are passed on to the IWM() function
# if silent is TRUE, progress report is suppressed
# a high iteration number and a small exp value..
...
>>>>>>> e03e536bbfa04053d7d01ada86cf3a92de889af2
